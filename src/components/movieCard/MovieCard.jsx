    ...
    <div className="w-full max-w-sm  mx-auto  rounded-lg shadow dark movieCard ">
      <div className="movieImg text-center relative aspect-[1/1.5] overflow-hidden">
        ...
        <img
          ...
          src={movie?.backdrop_path? `https://image.tmdb.org/t/p/original/${movie?.backdrop_path}`:avatar}
          alt="product image"
          />
          </Link>
        <div className="flex items-center justify-between mx-1 z-20 ">
          <div className="absolute left-2 bottom-2 z-10">
            <Rating rating={movie?.vote_average !==0?movie?.vote_average:"0.0"} width={50}/>
          </div>
          ...
          {movie?.genre_ids?.slice(0,2)?.map((g) => {
              ...
              <p key={g} className=" px-4 py-1 text-[12px] bg-hover text-white rounded-lg  capitalize ">{genre[g]?.name}</p> 
